            ...
            >
              ...
              <div class="product-details">
                ...
                <div class="product-rating">
                  <div v-if="isLoadingRating(product.productId || product.id)" class="rating-loading">
                    <span class="loading-text">Loading rating...</span>
                  </div>
                  <div v-else-if="getProductRating(product.productId || product.id) > 0" class="rating-display">
                    <span class="stars">{{ renderStarRating(getProductRating(product.productId || product.id)) }}</span>
                    <span class="rating-number">{{ getProductRating(product.productId || product.id).toFixed(1) }}/5</span>
                  </div>
...
export default {
  ...
  setup() {
    ...
    const productRatings = ref(new Map()) // Map to store ratings by productId
    const ratingsLoading = ref(new Set()) // Set to track which products are loading ratings
    ...
    const loadProductRatings = async () => {
      const productIds = allProducts.value.map(product => product.productId || product.id).filter(id => id)

      for (const productId of productIds) {
        if (!productRatings.value.has(productId)) {
          fetchProductRating(productId)
        }
      }
    }

    // Fetch rating for a specific product
    const fetchProductRating = async (productId) => {
      if (ratingsLoading.value.has(productId)) return

      ratingsLoading.value.add(productId)

      try {
        const averageRating = await api.getAverageRating(productId)
        productRatings.value.set(productId, averageRating || 0)
      } catch (error) {
        console.error(`Error fetching rating for product ${productId}:`, error)
        // Set 0 rating on error (no rating available)
        productRatings.value.set(productId, 0)
      } finally {
        ratingsLoading.value.delete(productId)
      }
    }

    // Get rating for a product
    const getProductRating = (productId) => {
      return productRatings.value.get(productId) || 0
    }

    // Check if rating is loading for a product
    const isLoadingRating = (productId) => {
      return ratingsLoading.value.has(productId)
    }
